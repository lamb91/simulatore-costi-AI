...
const DEFAULT_PRICES = {
  // ASR - Google Speech-to-Text (per minuto)
  google_asr_standard: 0.024,    // $/min standard
  google_asr_enhanced: 0.036,    // $/min enhanced

  // TTS - Google (per milione di caratteri)
  google_tts_standard: 4.0,      // $/1M chars
  google_tts_wavenet: 16.0,      // $/1M chars
  google_tts_neural2: 16.0,      // $/1M chars
  google_tts_studio: 30.0,       // $/1M chars

  // TTS - ElevenLabs (per 1000 caratteri)
  elevenlabs_flash_25: 0.06,     // $/1K chars
  elevenlabs_multi_v1: 0.12,     // $/1K chars
  elevenlabs_multi_v2: 0.12,     // $/1K chars

  // LLM - Gemini 2.5 Flash (per milione di token)
  gemini_flash_input: 0.30,      // $/1M tokens input
  gemini_flash_output: 2.50,     // $/1M tokens output

  // LLM - GPT-4.1 (per milione di token)
  gpt41_input: 2.00,             // $/1M tokens input
  gpt41_output: 8.00,            // $/1M tokens output
};
...
const PROFILES = [
  ...
  },
  {
    ...
    config: {
      ...
      avgDurationSec: 120,
      turnsPerConv: 4,
      asrModel: "google_asr_enhanced",
      ttsModel: "elevenlabs_flash_25",
      llmModel: "gpt41",
      avgInputTokens: 400,
      avgOutputTokens: 200,
      avgTtsChars: 150,
...
function ResultPanel({ results, config }) {
  const { asrCost, ttsCost, llmCost, totalCost, costPerConv, totalAsrMinutes, totalTtsChars, totalInputTokens, totalOutputTokens } = results;
  return (
    <div className="result-panel">
      <div style={{ display: "flex", alignItems: "baseline", gap: "12px", marginBottom: "4px" }}>
        <div className="result-total">{fmtEur(totalCost)}</div>
        <span style={{ fontSize: "14px", color: "var(--text-mid)" }}>/ mese</span>
      </div>
      <div className="result-sub">
        Costo per conversazione: <strong className="mono">{fmtEur(costPerConv)}</strong> &middot; {fmtInt(config.conversations)} conversazioni
      </div>
      ...
      <div style={{ fontSize: "13px", fontWeight: 600, color: "var(--navy)", marginBottom: "8px" }}>Stime estese</div>
      <div className="grid-3" style={{ marginBottom: 0 }}>
        <Metric label="Costo trimestrale" value={fmtEur(totalCost * 3)} sub="3 mesi" />
        <Metric label="Costo semestrale" value={fmtEur(totalCost * 6)} sub="6 mesi" />
        <Metric label="Costo annuale" value={fmtEur(totalCost * 12)} sub="12 mesi" color="orange" />
      </div>
    </div>
  );
}
...
function QuickMode({ prices }) {
  ...
  return (
    <>
          ...
          {PROFILES.map((p) => (
            <div
              ...
              onClick={() => {
                ...
                setConversations(p.config.conversations);
              }}
            >
              ...
              <div className="profile-specs">
                {p.specs.map((s, i) => (
                  <span key={i} className="profile-spec">{s}</span>
                ))}
              </div>
            </div>
          ))}
      ...
      <ResultPanel results={results} config={config} />
        ...
        <div className="grid-3" style={{ marginBottom: "1rem" }}>
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={config.ttsModel} onChange={(e) => update("ttsModel", e.target.value)}>
              {Object.entries(TTS_MODELS).map(([k, v]) => (
                <option key={k} value={k}>{v}</option>
              ))}
            </select>
          </div>
          <div className="form-group">
            ...
            <select className="form-select" value={config.llmModel} onChange={(e) => update("llmModel", e.target.value)}>
              {Object.entries(LLM_MODELS).map(([k, v]) => (
                <option key={k} value={k}>{v}</option>
              ))}
            </select>
          </div>
...
// ─── Comparison Mode ───
function CompareMode({ prices }) {
  const configs = useMemo(() => [
    {
      label: "Gemini Flash + Google WaveNet",
      config: {
        conversations: 10000, avgDurationSec: 90, turnsPerConv: 3,
        asrModel: "google_asr_standard", ttsModel: "google_tts_wavenet",
        llmModel: "gemini_flash", avgInputTokens: 300, avgOutputTokens: 150,
        avgTtsChars: 120, pctWithTts: 100,
      },
    },
    {
      ...
      config: {
        conversations: 10000, avgDurationSec: 90, turnsPerConv: 3,
        asrModel: "google_asr_standard", ttsModel: "elevenlabs_flash_25",
        llmModel: "gemini_flash", avgInputTokens: 300, avgOutputTokens: 150,
        avgTtsChars: 120, pctWithTts: 100,
      },
    },
    {
      ...
      config: {
        conversations: 10000, avgDurationSec: 90, turnsPerConv: 3,
        asrModel: "google_asr_standard", ttsModel: "google_tts_wavenet",
        llmModel: "gpt41", avgInputTokens: 300, avgOutputTokens: 150,
        avgTtsChars: 120, pctWithTts: 100,
      },
    },
    {
  ...
  return (
    <>
      <div className="section">
        ...
        <div className="section-desc">Stessi parametri di conversazione, modelli diversi. Utile per confrontare rapidamente le opzioni.</div>

        <div className="form-group">
          <div className="form-label">Conversazioni / mese (uguale per tutti)</div>
          <div className="slider-row">
            <input className="slider" type="range" min={500} max={200000} step={500} value={conversations} onChange={(e) => setConversations(Number(e.target.value))} />
            <span className="slider-val">{fmtInt(conversations)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid-2" style={{ marginBottom: "1.5rem" }}>
        {results.map((r, i) => {
          ...
          return (
            <div key={i} className="card" style={{ borderLeft: `5px solid ${isCheapest ? "var(--green)" : "var(--pale-navy)"}` }}>
              <div style={{ fontSize: "13px", fontWeight: 600, color: "var(--navy)", marginBottom: "8px" }}>
                {r.label}
                {isCheapest && <span style={{ marginLeft: "8px", fontSize: "11px", color: "var(--green)", fontFamily: "'JetBrains Mono', monospace" }}>PIU CONVENIENTE</span>}
              </div>

              <div style={{ fontSize: "24px", fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: isCheapest ? "var(--green)" : "var(--navy)" }}>
                {fmtEur(r.results.totalCost)}
              </div>
              <div style={{ fontSize: "12px", color: "var(--text-mid)", marginBottom: "10px" }}>
                {fmtEur(r.results.costPerConv)} / conversazione
              </div>

              <BreakdownBar asr={r.results.asrCost} tts={r.results.ttsCost} llm={r.results.llmCost} total={r.results.totalCost} />
      ...
      <div className="callout">
        <strong>Base comune:</strong> STT Google Standard, 90s durata media, 3 turni, 300 token input / 150 output, 120 caratteri TTS per risposta. Varia solo la combinazione LLM + TTS.
      </div>
    </>
  );
}
  ...
  return (
    <div>
      ...
      <div className="hero">
        <div className="hero-inner">
          ...
          <p className="hero-sub">
            Strumento interattivo per stimare i costi di ASR, TTS e LLM nei progetti di AI conversazionale. Pensato per i commerciali di Ellysse e per la preparazione di capitolati tecnici.
          </p>
        </div>
      </div>
      ...
      <div className="main-wrap">
        ...
        {/* Content */}
        {mode === "quick" && <QuickMode prices={prices} />}
        {mode === "advanced" && <AdvancedMode prices={prices} />}
        {mode === "compare" && <CompareMode prices={prices} />}
      </div>
